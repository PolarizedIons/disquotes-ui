<template>
  <div class="mx-auto w-3/4 my-5 flex justify-between">
    ...
    >
        ...
        <dq-button @click="submit()" bgColor="green-800" bgHoverColor="green-700" :busy="saving">Submit!</dq-button>
...
export default class Dashboard extends Vue {
  ...
  get guild(): Guild | null {
    return guildModule.guilds?.find(guild => guild.id === this.$route.params.guildId) || null;
  }
...
@Watch('newQuote', {deep: true})
  ...
  submit() {
    ...
    QuoteService.submitQuote(quote)
        .then((res) => {
            this.saving = false;
            if (!res.success) {
                ...
                this.$router.push({name: 'guild-dashboard', params: {guildId: this.guild?.id as string}})
            }
        });
